...
public class Activity_PlayMusic extends AppCompatActivity
{
    ...
    ImageButton imageButton_moreoption , imageButton_play, imageButton_previous, imageButton_next;
    ...
    private void handle_playmusic()
    {
        ...
        // set icon đã download
        if (isclick_fromoffline)
        {
            imageButton_download.setImageResource(R.drawable.cloud_downloaded);
            imageButton_download.setEnabled(false);
            isclick_fromoffline = false;
        }
        else if (check_duplicateId(dsid_BaiHatDaTaiXuong,baiHat_isplaying.getIdBaiHat()))
        {
            imageButton_download.setImageResource(R.drawable.cloud_downloaded);
        }
